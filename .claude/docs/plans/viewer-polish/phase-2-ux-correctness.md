...
- [ ] **Step 1 (extract)**: shortcuts currently live inline in
...
- [ ] **Step 3**: refactor the freshly-extracted
...
- [ ] Add `setToolTip(...)` calls in
  ...
  - Overlay combo: "Active event-overlay set; N / Shift+N navigate
    events."
- [ ] Test: filter the controls bar's children to *interactive*
...
- [ ] Add a helper `_available_overlay_modes(run: RunBundle) -> set[OverlayMode]`
...
- [ ] Update `_build_controls_bar` to enable/disable combo items
  via `model.item(i).setEnabled(...)`.
- [ ] Add per-item tooltip explaining the rebuild command (re-run
...
- [ ] Re-evaluate available modes in `_on_active_run_changed` (so
  M-key swap to a run with different outputs updates the combo).
- [ ] If the currently-selected mode becomes unavailable on a swap,
...
- [ ] In `QtLikelihoodHeatmapPanel`, replace `setTitle(MESSAGE)`
  with either:
  ...
  - A `pg.TextItem` anchored to the plot's center (acceptable —
    truncation easier).
- [ ] Reuse the same message string in the disabled-combo tooltip
...
- [ ] Test: build a panel for a run missing `log_likelihood`;
...
- [ ] Add `g` to `_SHORTCUT_TABLE` (category: `Navigation`) →
  `_show_go_to_time_dialog()`.
- [ ] Dialog: `QInputDialog.getDouble(...)` accepting `t` in seconds,
  with `min=time[0]`, `max=time[-1]`, `decimals=3`. On OK, call
  `self._core.set_t_center(t)`.
- [ ] Validation error message includes the valid range when out of
  bounds.
- [ ] Test: `g` shortcut bound; calling `_show_go_to_time_dialog`
