...
def _interpolate_value(place_bin_centers, likelihood, pos):
    value = scipy.interpolate.griddata(
        place_bin_centers, likelihood, pos, method="linear"
    )
    if np.isnan(value):
        value = scipy.interpolate.griddata(
...
def predict_sorted_spikes_kde_jax_log_likelihood(
    ...
    if is_local:
        ...
        for neuron_spikes, non_local_rate in zip(tqdm(spikes.T), place_fields):
            local_rate = _interpolate_value(place_bin_centers, non_local_rate, position)
