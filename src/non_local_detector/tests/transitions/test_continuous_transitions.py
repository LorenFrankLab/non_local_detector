...
def make_env_1d(n_bins=11, name="line"):
    env = Environment(environment_name=name, place_bin_size=1.0, position_range=((0.0, float(n_bins - 1)),))
...
def test_random_walk_row_stochastic_and_spread_var():
    ...
    envs = (env,)
    rw_tight = RandomWalk(environment_name=env.environment_name, movement_var=0.1).make_state_transition(envs)
    rw_wide = RandomWalk(environment_name=env.environment_name, movement_var=10.0).make_state_transition(envs)
...
def test_random_walk_with_directional_constraints():
    ...
    envs = (env,)
    base = RandomWalk(environment_name=env.environment_name, movement_var=3.0).make_state_transition(envs)
    up = RandomWalkDirection1(environment_name=env.environment_name, movement_var=3.0).make_state_transition(envs)
    down = RandomWalkDirection2(environment_name=env.environment_name, movement_var=3.0).make_state_transition(envs)

    # Directional versions are masked upper/lower triangular
    assert np.allclose(up, np.triu(base) / np.clip(np.triu(base).sum(axis=1, keepdims=True), 1e-12, None))
    assert np.allclose(down, np.tril(base) / np.clip(np.tril(base).sum(axis=1, keepdims=True), 1e-12, None))
...
def test_uniform_transition_between_envs():
    ...
    envs = (env1, env2)
    tm = Uniform(environment_name=env1.environment_name, environment2_name=env2.environment_name).make_state_transition(envs)
    assert tm.shape == (env1.place_bin_centers_.shape[0], env2.place_bin_centers_.shape[0])
    assert rows_sum_to_one(tm)
