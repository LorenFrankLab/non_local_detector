...
if TYPE_CHECKING:  # avoid run-time circular dep
    ...
    from non_local_detector.environment.core import Environment
...
class LayoutResult:
    ...
    This structure holds all necessary information generated by a `LayoutEngine`
    to define the geometry, connectivity, and metadata of a discretized
    environment. It serves as the input to `Environment.from_layout`.

    Attributes
    ----------
    bin_centers_ : NDArray[np.float64]
        Coordinates of the center of each *active* bin. Shape (n_active_bins, n_dims).
    connectivity_graph_ : nx.Graph
        Graph where nodes are integers from `0` to `n_active_bins - 1`,
        representing active bins and their adjacency.
    ...
    is_1d_ : bool
        True if the layout represents a primarily 1-dimensional structure, False otherwise.
    point_to_bin_func_ : PointToBin
        A callable that maps N-D continuous points to discrete active bin indices.
    active_bin_areas_ : NDArray[np.float64]
        An array containing the area/size for each active bin. Shape (n_active_bins,).
    grid_edges_ : Optional[Tuple[NDArray[np.float64], ...]], default=None
        ...
        full grid*.
    grid_shape_ : Optional[Tuple[int, ...]], default=None
        For grid-based layouts: The N-D shape (number of bins in each
        dimension) of the *original, full grid*.
    active_mask_nd_ : Optional[NDArray[np.bool_]], default=None
        For grid-based layouts: An N-D boolean mask indicating active bins
        on the *original, full grid* (shape matches `grid_shape_`).
    plot_layout_func_ : Optional[PlotFunc], default=None
        An optional callable to plot the general layout of the environment.
    plot_1d_layout_func_ : Optional[PlotFunc], default=None
        An optional callable to plot a 1D-specific representation of the layout.
    graph_definition_ : Optional[nx.Graph], default=None
        For 1D graph-based layouts: The original `nx.Graph` used to define the track.
    edge_order_ : Optional[Tuple[Tuple[int, int], ...]], default=None
        For 1D graph-based layouts: Ordered sequence of edge tuples defining
        the linearized track segments.
    edge_spacing_ : Optional[float], default=None
        For 1D graph-based layouts: Spacing (gap length) between consecutive edges.
    lin_func_ : Optional[LinearProjectionFn], default=None
        For 1D layouts: Function mapping bin indices to linear positions.
    lin_to_nd_func_ : Optional[LinearToBinFn], default=None
        For 1D layouts: Function mapping linear positions to bin indices.
    nd_points_to_linear_func_ : Optional[LinearProjectionFn], default=None
        For 1D layouts: Function projecting N-D points to continuous linear positions.
    """
    ...
    def __post_init__(self: LayoutResult) -> None:
        """
        Validate the LayoutResult instance after initialization.

        Raises
        ------
        ValueError
            If `bin_centers_` is not 2D.
            If `connectivity_graph_` node count mismatches `bin_centers_`.
            If `active_mask_nd_` shape mismatches `grid_shape_`.
        TypeError
            If mandatory callable fields (e.g., `point_to_bin_func_`) are not callable.
        """
...
class _GridMixin:
    ...
    def get_layout_result(self) -> LayoutResult:
        if (
            self.bin_centers_ is None or self.connectivity_graph_ is None
        ):  # Or a more specific check
            raise RuntimeError(
                f"{self._layout_type_tag} layout has not been built. Call 'build()' first."
            )

        # Wrapper for point_to_bin_func_
        def _point_to_bin_for_env(env_obj: Environment, points: PtArr) -> IdxArr:
            # The env_obj argument is part of the LayoutResult's callable signature,
            # but the layout engine's own method might not need it directly.
            return self.point_to_bin_index(points)

        # Wrapper for plot_layout_func_
        def _plot_for_env(env_obj: Environment, ax: MatplotlibAxes, **kwargs) -> None:
            self.plot(ax=ax, **kwargs)

        # Determine active_mask_nd_ for LayoutResult
        # For most grid-based layouts, self.active_mask_ is the N-D mask of the original grid.
        active_mask_nd_for_result = self.active_mask_

        return LayoutResult(
            bin_centers_=self.bin_centers_,
            connectivity_graph_=self.connectivity_graph_,
            dimension_ranges_=self.dimension_ranges_,
            is_1d_=self.is_1d,  # Property from the engine
            point_to_bin_func_=_point_to_bin_for_env,
            active_bin_areas_=self.get_bin_area_volume(),
            grid_edges_=self.grid_edges_,
            grid_shape_=self.grid_shape_,
            active_mask_nd_=active_mask_nd_for_result,
            plot_layout_func_=_plot_for_env,
            # 1D specific attributes will be None for non-GraphLayouts
            plot_1d_layout_func_=None,
            graph_definition_=None,
            edge_order_=None,
            edge_spacing_=None,
            lin_func_=None,
            lin_to_nd_func_=None,
            nd_points_to_linear_func_=None,
        )
...
class HexagonalLayout(_KDTreeMixin):
    ...
    def get_layout_result(self) -> LayoutResult:
        """Packages the hexagonal layout's properties into a LayoutResult object."""
        if (
            self.bin_centers_ is None
            or self.connectivity_graph_ is None
            or self.dimension_ranges_ is None
            or self.active_mask_ is None
        ):
            raise RuntimeError(
                f"{self._layout_type_tag} layout has not been built. Call 'build()' first."
        ...
        return LayoutResult(
            ...
            grid_edges_=self.grid_edges_,
            grid_shape_=self.grid_shape_,
            active_mask_nd_=self.active_mask_,
            plot_layout_func_=_plot_for_env,
        )
...
class GraphLayout(_KDTreeMixin):
    ...
    def get_layout_result(self) -> LayoutResult:
        if (
            self.bin_centers_ is None
            or self.connectivity_graph_ is None
            or self.linear_bin_centers_ is None
        ):
            raise RuntimeError(
                f"{self._layout_type_tag} layout has not been built. Call 'build()' first."
            )

        # Wrapper for point_to_bin_func_ (maps N-D points to active bin indices)
        def _point_to_bin_for_env(env_obj: Environment, points: PtArr) -> IdxArr:
            return self.point_to_bin_index(points)  # Uses KDTree on N-D embeddings

        # Wrapper for plot_layout_func_ (plots N-D embedding)
        def _plot_for_env(env_obj: Environment, ax: MatplotlibAxes, **kwargs) -> None:
            self.plot(ax=ax, **kwargs)

        # Wrapper for plot_1d_layout_func_
        def _plot_1d_for_env(
            env_obj: Environment, ax: MatplotlibAxes, **kwargs
        ) -> None:
            self.plot_linear_layout(ax=ax, **kwargs)

        # Wrapper for lin_func_ (active bin_indices to linear positions)
        def _lin_func_for_env(env_obj: Environment, bin_indices: IdxArr) -> PtArr:
            # self.active_mask_ is 1D, True for active bins in linear_bin_centers_
            # self.linear_bin_centers_ contains all 1D bin centers (active and inactive)
            active_linear_centers = self.linear_bin_centers_[self.active_mask_]
            return active_linear_centers[bin_indices]

        # Wrapper for lin_to_nd_func_ (linear positions to active bin_indices)
        def _lin_to_nd_func_for_env(
            env_obj: Environment, linear_positions: PtArr
        ) -> IdxArr:
            return self.linear_point_to_bin_ind(linear_positions)

        # Wrapper for nd_points_to_linear_func_ (N-D points to continuous linear positions)
        def _nd_points_to_linear_func_for_env(
            env_obj: Environment, points_nd: PtArr
        ) -> PtArr:
            return self.get_linearized_coordinate(points_nd)

        # Handle edge_spacing for LayoutResult (expects Optional[float])
        graph_edge_spacing = self._build_params_used.get("edge_spacing")
        layout_result_edge_spacing: Optional[float] = None
        if isinstance(graph_edge_spacing, (float, int)):
            layout_result_edge_spacing = float(graph_edge_spacing)
        # If it's a list/sequence, LayoutResult expects a float, so we pass None.
        # Alternatively, you could average it or take the first element if appropriate,
        # but None is safer if the meaning of a single float is ambiguous for list spacing.

        return LayoutResult(
            bin_centers_=(
                self.bin_centers_[self.active_mask_]
                if self.is_1d
                else self.bin_centers_
            ),  # Ensure only active N-D centers
            connectivity_graph_=self.connectivity_graph_,
            dimension_ranges_=self.dimension_ranges_,
            is_1d_=self.is_1d,
            point_to_bin_func_=_point_to_bin_for_env,
            active_bin_areas_=self.get_bin_area_volume(),  # Lengths for active 1D bins
            # Grid-like properties for the 1D linearization
            grid_edges_=self.grid_edges_,  # Tuple containing 1D array of linearized bin edges
            grid_shape_=(
                len(self.linear_bin_centers_),
            ),  # Shape of the 1D linearized "grid"
            active_mask_nd_=self.active_mask_,  # 1D boolean mask for linearized bins
            plot_layout_func_=_plot_for_env,
            plot_1d_layout_func_=_plot_1d_for_env,
            graph_definition_=self._build_params_used.get("graph_definition"),
            edge_order_=tuple(
                map(tuple, self._build_params_used.get("edge_order", []))
            ),
            edge_spacing_=layout_result_edge_spacing,
            lin_func_=_lin_func_for_env,
            lin_to_nd_func_=_lin_to_nd_func_for_env,
            nd_points_to_linear_func_=_nd_points_to_linear_func_for_env,
        )
...
def create_layout_result(kind: str, **kwargs) -> LayoutResult:  # Renamed for clarity
    """
    Instantiate, build, and get a LayoutResult from a layout engine.

    Parameters
    ----------
    kind : str
        The string identifier of the layout engine type.
    **kwargs : Any
        Keyword arguments for the engine's `build` method.

    Returns
    -------
    LayoutResult
        The resulting layout specification.
    """
    ...
    # Filter kwargs to only those accepted by the build method
    ...
    return engine_instance.get_layout_result()  # Call the new method
